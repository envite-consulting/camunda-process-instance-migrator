...
public class MigrationInstructionCombiner {
  ...
  /**
   * Combines a list of {@link MinorMigrationInstructions} into a list of migration-ready {@link
   * CustomMigrationInstruction}, thereby taking into account possible 'adding' of instructions that
   * span multiple minor versions.
   *
   * @param applicableMinorMigrationInstructions the migration instructions that are to be combined.
   * @return a list of {@link CustomMigrationInstruction} containing all combined instructions from
   *     the input.
   */
                ...
                minorMigrationInstructions.getMigrationInstructions().stream()
                    ...
                    .forEach(
                        migrationInstruction -> {
                          // check if a migration instruction exists, that has that
                          // migrationInstructions
                          // source as a target, i.e. instruction 1 goes from activity1 to activity2
                          // and
                          ...
                          if (migrationInstructionWasAlreadySet && instructionToReplace != null) {
                            ...
                            instructionList.add(
                                new CustomMigrationInstruction(
                                    instructionToReplace.getSourceActivityId(),
                                    migrationInstruction.getTargetActivityId(),
                                    true));
                          }
                          // if the instruction does not need to be combined, just add it to the
                          // list
                          else {
                            instructionList.add(
                                new CustomMigrationInstruction(
