...
class ProcessVersionTest {
  ...
  @ValueSource(
      strings = {
        "1.3,0", "1.3.", "1.3.0.2", "1..3", ".1.3", "13..", "1.3..4", "a.1.3", "1.b.c", "1.2.%"
      })
  ...
  void fromString_toVersionTag_should_result_in_original_String_for_valid_version_tags(
      String versionTagString) {
    assertThat(ProcessVersion.fromString(versionTagString).get().toVersionTag())
        .isEqualTo(versionTagString);
  }

  @Test
  void isOlderVersionThan_should_return_true_for_lower_major_version() {
    assertThat(new ProcessVersion(2, 3, 5).isOlderVersionThan(new ProcessVersion(3, 1, 2)))
        .isTrue();
  }

  @Test
  void isOlderVersionThan_should_return_true_for_lower_minor_version() {
    assertThat(new ProcessVersion(2, 3, 5).isOlderVersionThan(new ProcessVersion(2, 4, 2)))
        .isTrue();
  }

  @Test
  void isOlderVersionThan_should_return_true_for_lower_patch_version() {
    assertThat(new ProcessVersion(2, 3, 5).isOlderVersionThan(new ProcessVersion(2, 3, 7)))
        .isTrue();
  }

  @Test
  void isOlderVersionThan_should_return_false_for_higher_major_version() {
    assertThat(new ProcessVersion(2, 3, 5).isOlderVersionThan(new ProcessVersion(1, 4, 7)))
        .isFalse();
  }

  @Test
  void isOlderVersionThan_should_return_false_for_higher_minor_version() {
    assertThat(new ProcessVersion(2, 3, 5).isOlderVersionThan(new ProcessVersion(2, 2, 7)))
        .isFalse();
  }

  @Test
  void isOlderVersionThan_should_return_false_for_higher_patch_version() {
    assertThat(new ProcessVersion(2, 3, 5).isOlderVersionThan(new ProcessVersion(2, 3, 4)))
        .isFalse();
  }

  @Test
  void isOlderVersionThat_should_return_false_for_equal_versions() {
    assertThat(new ProcessVersion(2, 3, 5).isOlderVersionThan(new ProcessVersion(2, 3, 5)))
        .isFalse();
  }
